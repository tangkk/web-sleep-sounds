/* Independent stochastic tracks. No shared background layers or fixed loops. */
(() => {
'use strict';
const rnd=(a,b)=>a+Math.random()*(b-a);
const clamp=(x,a,b)=>Math.max(a,Math.min(b,x));
// Small blocks keep first playback responsive on mobile; each block is newly generated.
const LENGTH=4,OVERLAP=.35,STEP=LENGTH-OVERLAP;
const types={
...
function bufferFor(context,id){
 const sr=context.sampleRate,length=Math.ceil(sr*LENGTH),buffer=context.createBuffer(1,length,sr),data=buffer.getChannelData(0),type=types[id];
 if(type.color){
  ...
  // Only individual rain drops, wood cracks or water bubbles; no noise bed.
  let t=rnd(0,.04),rate=rnd(.85,1.15);
  while(t<LENGTH){
  ...
  while(t<LENGTH){
   ...
   for(let i=begin;i<end;i++){
    const p=(i/sr-t)/duration,peak=id==='ocean'?.38:.28;
    const shape=p<peak?Math.pow(Math.max(0,p)/peak,1.35):Math.pow(Math.max(0,(1-p)/(1-peak)),1.6);
  ...
  for(let i=0;i<length;i++){
   const w=Math.random()*2-1;
   slow=slow*.985+w*.015;fast=fast*.65+w*.35;
   data[i]=(id==='ocean'?slow*3.1+fast*.16:slow*1.9+fast*.3)*envelope[i]*.55;
  }
 }
...
window.createNaturalSoundEngine=context=>{
 ...
 function schedule(track){
  // Schedule just one fresh block at a time. Keeping the first buffer short
  // avoids generating minutes of unused audio before context.resume().
  if(track.nextStart>=context.currentTime+1.1)return;
  const start=Math.max(track.nextStart,context.currentTime+.02);
  const source=context.createBufferSource();source.buffer=bufferFor(context,track.id);
  const fade=context.createGain();
  const attack=track.first?.045:OVERLAP;
  fade.gain.setValueAtTime(0,start);
  fade.gain.linearRampToValueAtTime(1,start+attack);
  ...
  track.first=false;track.nextStart=start+STEP;
 }
 function move(track){
 ...
 function tick(){if(context.state!=='running')return;for(const track of tracks.values()){schedule(track);move(track);}}
 function create(id,output){
  ...
  const track={id,type,bus,texture,low,nextStart:context.currentTime+.02,lastMove:context.currentTime,nextDelay:rnd(3,8),target:.75,first:true};
  tracks.set(id,track);
  // Initial block is short and starts with an almost immediate 45 ms fade.
  schedule(track);
